...
TEST_CASE("avx2_memchr", "memchr") {
    ...
    CHECK(static_cast<const char *>(memchr_avx2(line.data(), '2', line.size())) ==
          (line.data() + 2));
    CHECK(static_cast<const char *>(memchr_avx2(line.data(), ' ', line.size())) ==
          (line.data() + 11));
    CHECK(static_cast<const char *>(memchr_avx2(line.data(), 'W', line.size())) ==
          (line.data() + 116));

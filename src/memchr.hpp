...
const char *memchr_avx2_v2(const char *ptr, const char c, const size_t len) {
	...
	if ((((size_t)idx & 127u) != 0) && (idx < last_pos - 64)) {
		...
		if (mask1) return idx + __builtin_ffs(mask1) - 1;
		else if (mask2) return idx + __builtin_ffs(mask2) + 31;
	...
	if (idx < last_pos_m128) {
		...
		else if (mask3) return idx + __builtin_ffs(mask3) + 63;
		else return idx + __builtin_ffs(mask4) + 95; // mask4 
	}
